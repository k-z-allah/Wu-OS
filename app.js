...
const views = {
  ...
  browser(){ return `<div class="browser-shell"><form id="browserForm" class="browser-bar"><input id="browserAddress" inputmode="url" placeholder="Search or enter website" value="https://example.com"><button>GO</button></form><div id="browserView" class="browser-view"><div class="browser-placeholder"><b>36 Browser</b><p>Enter a URL. Some websites block embedding; use “Open External” when that happens.</p><button id="openExternal" class="primary">OPEN EXTERNAL</button></div></div></div>` },
...
function bindApp(id){
  ...
  if(id==='browser'){
    const input=$('#browserAddress'); const load=()=>{let url=input.value.trim();if(!/^https?:\/\//i.test(url))url='https://www.google.com/search?q='+encodeURIComponent(url);input.value=url;$('#browserView').innerHTML=`<iframe sandbox="allow-forms allow-scripts allow-same-origin allow-popups" referrerpolicy="no-referrer" src="${url.replace(/"/g,'&quot;')}"></iframe>`;};
    $('#browserForm').onsubmit=e=>{e.preventDefault();load()}; $('#openExternal').onclick=()=>window.open(input.value,'_blank','noopener,noreferrer');
  }
  if(id==='music'){
