...
- Browser shell with sandboxed iframe and external-open fallback
...
## Browser limitation
Many websites send `X-Frame-Options` or CSP headers that prevent iframe embedding. The browser app therefore includes an “Open External” fallback. A production version should use a native `WKWebView` wrapper or open links in Safari.
